...
# key = "$2a$10$T280RAjQ0OyHCYwUh.9iku4SiDlfIvf/zxl6nbXKFFDAZla8NLLAG"
...
def handle_missing_values(df, datatype_df):
    ...
    replace_values = [' ', '_', 'NA', '.', 'na', 'none', 'None', 'NONE']
...
def data_cleaning(df, datatype_df):
    ...
    for col in df.columns:
        if df[col].nunique() == 1:  # If the column has only one unique value
            ...
            print(f"Column '{col}' removed as it has only one unique value.")
...
if page == "Analysis & Report Generation":
    ...
    if uploaded_file is not None:
        try:
            ...
            if identifier_index < len(columns_of_dataset):
